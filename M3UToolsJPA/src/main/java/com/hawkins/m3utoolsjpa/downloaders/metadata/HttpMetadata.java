...
public class HttpMetadata {
	...
	public static HttpMetadata load(String id) {
		...
		try {
			...
			if (key.equals("type")) {
				...
				if (type == DManagerConstants.HTTP) {
					metadata = new HttpMetadata(id);
				} else if (type == DManagerConstants.HLS) {
					metadata = new HlsMetadata(id);
				} else if (type == DManagerConstants.HDS) {
					metadata = new HdsMetadata(id);
				} else if (type == DManagerConstants.DASH) {
					metadata = new DashMetadata(id);
				}
			} else {
				log.info("invalid metadata file starting with: " + ln);
	...
	public void save() {
		...
		try {
			...
			if (getType() == DManagerConstants.HDS) {
				sb.append("bitrate: " + ((HdsMetadata) this).getBitRate() + "\n");
			}
			if (getType() == DManagerConstants.DASH) {
