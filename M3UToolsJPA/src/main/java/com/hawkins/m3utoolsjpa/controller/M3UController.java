...
public class M3UController {
    ...
    public String getAll(Model model, @RequestParam(required = false, defaultValue = "-1") Long groupId,
        ...
        try {
            ...
            M3UGroupSelected selectedGroupData = m3uService.getSelectedGroup(groupId);
            List<M3UGroup> groups = m3uService.getM3UGroupsByType(Constants.MOVIE);

            model.addAttribute(Constants.MOVIEDB, MovieDb.getInstance());
            model.addAttribute(Constants.SELECTEDGROUP, selectedGroupData);
